...
fn main() {
  ...
  impl HitTestResult {
    fn drag_resize_window(&self, window: &winit::window::Window) {
    ...
    fn change_cursor(&self, window: &winit::window::Window) {
      window.set_cursor(match self {
  ...
  fn hit_test(window_size: PhysicalSize<u32>, x: i32, y: i32, scale: f64) -> HitTestResult {
    ...
    let result = (LEFT * (if x < (left + inset) { 1 } else { 0 }))
...
#[cfg(target_os = "linux")]
fn main() {
  println!("This example is not supported on Linux!");
}
